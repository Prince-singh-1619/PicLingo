...
const Upload = () => {
  ...
  return (
    <section className='h-full w-full flex flex-col jutify-center gap-2 items-center py-15'>
      ...
      <p className='text-xl max-sm:text-sm text-slate-300 mt-2 text-center'>Upload an image to get AI-powered caption recommendations.</p>
      <div className='min-h-[420px] max-sm:h-[500px] min-w-[55%] max-sm:w-[95%] border border-slate-700 rounded-lg mt-10 flex flex-col justify-around items-center gap-4'>
        <div className='p-10 w-[95%] max-sm:w-[80%] border border-dashed border-slate-500 rounded-lg flex justify-center items-center'>
          {
            imageArray.length===0 ? (
                <div className='flex flex-col justify-center items-center gap-4  '>
                  <div className='h-18 w-18 sm:h-20 sm:w-20 rounded-full flex justify-center items-center text-4xl bg-[#1a1a1a] text-slate-400'><LuImage/></div>
        ...
        <button className='w-84 max-sm:w-70 text-center'>
          {
            imageArray.length !== 0 ? (
              <div onClick={genCaps}><span className='btn block text-white/87'>Generate Captions</span></div>
            ) : (
